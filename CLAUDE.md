...
- **Swappable backend** via `BuildConfig` flags in `app/build.gradle.kts`:
  - `USE_MOCK_AUTH` / `USE_MOCK_BACKEND` = `true` in **debug** (use `MockAuthService` / `BackendServiceStub`), `false` in **release** (real Firebase). **Do not flip these yet:** 14 `FirebaseBackendService` methods return `Result.Error(Exception("…not yet implemented"))` (`:484–524`) — see `TECH_LEAD_REVIEW.md` §9 and `ROADMAP.md` Track C. (Firestore reads themselves now work: every DTO param has a default so the object mapper can construct it, pinned by `FirestoreDtoContractTest`. Because that also lets a malformed document deserialize into blanks, `*Dto.toDomain()` validates ids and enums and throws `MalformedDtoException` — don't add a DTO mapper that silently defaults an unparseable enum.)
