...
| Issue                                                                                                                                                | File                                          |
|------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|
| ✅ RESOLVED — DTOs had required (no-default) params, so Kotlin emitted no no-arg constructor and all 26 `toObject()`/`toObjects()` calls threw into a swallowed `Result.Error`: a **write-only app**. Every param now has a default (guarded by `FirestoreDtoContractTest`), and the `*Dto.toDomain()` mappers validate ids/enums so a malformed document fails loudly instead of deserializing into blanks | `IBackendService.kt`, `Mappers.kt` |
| 14 `FirebaseBackendService` methods return `Result.Error(Exception("…not yet implemented"))`                                                                     | `FirebaseBackendService.kt:484–524`           |
| ✅ RESOLVED — signup creates the profile document and honours the `role`, done in `AuthRepository` so it covers both backends. Remaining gap: the credential and the profile are two non-atomic writes, so a profile-write failure orphans the auth account (reported as an error, not swallowed) | `AuthRepository.kt`, `FirebaseAuthService.kt` |
...
| `getCurrentClient()` returns `Result.Error(Exception("Not implemented"))`; two `PaymentRepository` methods and one `ServiceRepository` method return `"Not implemented in backend service"`. (`updateProvider()` ✅ fixed — it calls the new `IBackendService.updateProvider`.) | `ClientRepository.kt:87`, `PaymentRepository.kt:119,132`, `ServiceRepository.kt:151` |
